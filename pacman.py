...
INITIAL_DESC = "Welcome to the Birney group"
HS_FONT_SIZE = 18
HS_LINE_HEIGHT = 20
HS_WIDTH = 600
HS_HEIGHT = 150
...
def main():
    ...
    try:
        ...
        try:
            script_path = input('For some reason, we could not infer the path of the game, please input the directory where pacman.py is located:')
            img_Background = get_image_surface(os.path.join(script_path, "res", "backgrounds", "1.gif"))
    ...
    class game:
        ...
        #     hs = self.gethiscores() # Here we nee to change for the dict
        #     hs_id, hs_info, hs_pic = hs.split('__')
        #     vpos = 0
        #     picture = pygame.image.load(os.path.join(script_path, "res", "pictures", hs_pic[:-1] + '.png'))
        #     vpos += HS_LINE_HEIGHT
        #     linesurf = f.render(hs_info.ljust(18), 1, (255, 255, 255))
        #     scoresurf.blit(picture, (0, vpos))
        #     scoresurf.blit(linesurf, (110, vpos))
        #
        #     # for line in hs:
        ...
        def AddToScore(self, amount):
            ...
            for specialScore in extraLifeSet:
                if self.score < specialScore <= self.score + amount:
                    snd_extralife.set_volume(SOUND_VOLUME)
                    snd_extralife.play()
                    thisGame.lives += 1
    ...
    class fruit:
        ...
        def Move(self):
            ...
            if self.bouncei == 1:
                ...
                self.bounceY = 0
                self.bouncei = 0
                snd_fruitbounce.set_volume(SOUND_VOLUME)
        ...
        def __init__(self):
            ...
            for i in range(1, 9, 1):
                self.anim_pacmanL[i] = get_image_surface(
                    os.path.join(script_path, "res", "sprite", "Ewan-l.gif"))
                self.anim_pacmanR[i] = get_image_surface(
                    os.path.join(script_path, "res", "sprite", "Ewan-r.gif"))
                self.anim_pacmanU[i] = get_image_surface(
                    os.path.join(script_path, "res", "sprite", "Ewan-u.gif"))
                self.anim_pacmanD[i] = get_image_surface(
                    os.path.join(script_path, "res", "sprite", "Ewan-d.gif"))
                self.anim_pacmanS[i] = get_image_surface(os.path.join(script_path, "res", "sprite", "Ewan-r.gif"))
            ...
            if not thisLevel.CheckIfHitWall((self.x + self.velX, self.y + self.velY), (self.nearestRow, self.nearestCol)):
                ...
                for i in range(0, 4, 1):
                    if thisLevel.CheckIfHit((self.x, self.y), (ghosts[i].x, ghosts[i].y), TILE_WIDTH / 2):
                        ...
                        if ghosts[i].state == 1:
                            ...
                            snd_eatgh.set_volume(SOUND_VOLUME)
                            snd_eatgh.play()
                ...
                if thisFruit.active:
                    if thisLevel.CheckIfHit((self.x, self.y), (thisFruit.x, thisFruit.y), TILE_WIDTH / 2):
                        ...
                        snd_eatfruit.set_volume(SOUND_VOLUME)
                        snd_eatfruit.play()
    ...
    class level:
        def __init__(self):
            ...
            self.imHiscores = self.makehiscorelist(0)
                ...
                for iCol in range(col - 1, col + 2, 1):
                    ...
                    if (playerX - (iCol * TILE_WIDTH) < TILE_WIDTH) and (
                        ...
                        if result == tileID['pellet']:
                            ...
                            snd_pellet[player.pelletSndNum].set_volume(SOUND_VOLUME)
                        ...
                        if result == tileID['pellet-power']:
                            ...
                            thisGame.SetMode(9)
                            thisLevel.SetMapTile((iRow, iCol), 0)
                            snd_powerpellet.set_volume(SOUND_VOLUME)
                            snd_powerpellet.play()

                            #Trials
                            # thisLevel.drawmidgamehiscores(result)
                            thisGame.DrawScore()
                            thisGame.AddToScore(100)
                            thisGame.ghostValue = 200

                            thisGame.ghostTimer = 180 #360
                            for i in range(0, 4, 1):
                                if ghosts[i].state == 1:
                                    ghosts[i].state = 2


                        elif result in range(tileID['pellet-power'] + 1, tileID['pellet-power'] + 14):
                            # got a power pellet
                            thisGame.SetMode(12)
                            thisLevel.SetMapTile((iRow, iCol), 0)
                            snd_powerpellet.set_volume(SOUND_VOLUME)
        ...
        def gethiscores(self, results):
            """If res/hiscore.txt exists, read it. If not, return the default high scores.
               Output is [ (score,name) , (score,name) , .. ]. Always 6 entries."""
            f = open(os.path.join(script_path, "res", "hiscore.txt"))
            f_lines = f.readlines()
            hs = infoIDName[results]
            f.close()
            return hs

        def makehiscorelist(self, results):
            global rect_list
            "Read the High-Score file and convert it to a useable Surface."
            # My apologies for all the hard-coded constants.... -Andy
            f = pygame.font.Font(os.path.join(script_path, "res", "VeraMoBd.ttf"), HS_FONT_SIZE)
            ...
            # if len(hs_info) >
            vpos = 0
            picture = pygame.image.load(os.path.join(script_path, "res", "pictures", hs_pic + '.png'))
            vpos += HS_LINE_HEIGHT
            # Cutting the sentence to fit
            if len(hs_info) > 34:
                list_str = hs_info.split(' ')
                new_liststr = []
                line = ''
                for n in range(len(list_str)):
                    if len(line) + len(list_str[n]) < 34:
                        line = ' '.join([line, list_str[n]])
                    else:
                        new_liststr.append(line)
                        line = list_str[n]
                new_liststr.append(line)
            else:
                new_liststr = [hs_info]
            # if len(new_liststr) > 4:
            #     print("Text too long for tile", results, ': ', hs_info)
            linesurf = [f.render(l.ljust(52), 1, (255, 255, 255)) for l in new_liststr]
            scoresurf.blit(picture, (0, vpos - 15))
            for l, line in enumerate(linesurf): scoresurf.blit(line, (130, 25*(l+1)))

            return scoresurf

        def drawmidgamehiscores(self, results):
            """Redraw the high-score list image after pacman dies."""
            self.imHiscores = self.makehiscorelist(results)
    ...
    def CheckInputs():
        ...
        #     if pygame.key.get_pressed()[pygame.K_RETURN] or (js is not None and js.get_axis(JS_XAXIS) > 0.5):
                if not (player.velX == player.speed and player.velY == 0) and not thisLevel.CheckIfHitWall(
                        (player.x + player.speed, player.y), (player.nearestRow, player.nearestCol)):
                    player.velX = player.speed
                    player.velY = 0
    ...
    def GetHS():
        f = open(os.path.join(script_path, "res", "hiscore.txt"), 'r')
        ...
        for i in f.readlines():
            ...
            if useLine:
                infoIDName[int(str_splitBySpace[0])] = str_splitBySpace[1:]
                infoID[str_splitBySpace[1]] = int(str_splitBySpace[0])

                thisID = int(str_splitBySpace[0])
                if not thisID in NO_GIF_TILES:
                    infoIDImage[thisID] = get_image_surface(
                        os.path.join(script_path, "res", "tiles", str_splitBySpace[2] + ".gif"))
                else:
                    infoIDImage[thisID] = pygame.Surface((TILE_WIDTH, TILE_HEIGHT))
    ...
    infoIDName = {}  # gives tile name (when the ID# is known)
    infoID = {}  # gives tile ID (when the name is known)
    infoIDImage = {}  # gives tile image (when the ID# is known)
    ...
    while True:
        ...
        if thisGame.mode == 1:
            ...
            if thisGame.modeTimer < 100:
                screen.blit(thisLevel.imHiscores, (HS_XOFFSET, HS_YOFFSET))
            else:
                thisGame.SetMode(9)
        ...
        if not thisGame.mode == 10:
            ...
            if thisGame.mode == 3 :
                thisLevel.imHiscores = thisLevel.makehiscorelist(0)
                screen.blit(thisLevel.imHiscores, (HS_XOFFSET, HS_YOFFSET))
